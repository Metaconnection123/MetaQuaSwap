...
export const TabBtn = styled.span`
  ...
  border-bottom: ${({ isOpened }) =>
    isOpened ? "none" : " 2px solid #07588A"};
  border-radius: ${({ tabName }) =>
    tabName === "Tab1" ? "10px 0 0 0" : "0 10px 0 0"};
`;
...
export const Web3ButtonLayout = styled.div`
  display: flex;
  justify-content: center;
  align-items: center;
  height: 10%; // 만약 이 div가 부모 요소를 꽉 채우지 않는다면 이 속성을 추가하세요.
`;
